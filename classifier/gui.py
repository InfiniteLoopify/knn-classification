...
class Table:
    ...
    def display_Table(self, result, param, label, label_id, classes_names):
        ...
        for i, ans in enumerate(result):
            ...
            right_str = (
                "0" * (3 - len(str(int(label_id[i])))) + str(int(label_id[i])) + ".txt"
            )
    ...
    def create_Gui(self, indexer):
        ...
        self.label = tk.Label(self.scores, text="KNN", font=("Arial", 30)).grid(
            row=0, columnspan=2
        )
        ...
        self.label = tk.Label(
            self.scores, text="KNN Value:\t" + "3", font=("Arial", 10)
        ).place(x=694 + 160, y=48 - 40)
        self.label = tk.Label(
            self.scores,
            text="Training Data:\t" + str(int(indexer.param[0] * 100)) + "%",
            font=("Arial", 10),
        ).place(x=694 + 160, y=48 - 20)
        self.label = tk.Label(
            self.scores,
            text="Testing Data:\t" + str(int(100 - indexer.param[0] * 100)) + "%",
            font=("Arial", 10),
        ).place(x=694 + 160, y=48 - 0)
        self.label = tk.Label(
            self.scores,
            text=f"{indexer.param[3]:0.2f}%",
            fg="#275A2C",
            font=("Helvetica", 30),
        ).place(x=694 + 160, y=545)
        ...
        self.listBox.column(self.cols[1], minwidth=120, width=180, stretch=tk.NO)
        ...
        self.display_Table(
            ...
            indexer.param,
            indexer.test_label,
            indexer.test_label_id,
            indexer.classes_names,
        )
